...
| banner.gamma_blocked *(replaced banner.autobrightness 2026-09-10; uz/ru need human review)* | On macOS 26 an Apple bug can stop Dimit from changing your screen's colours on some Macs. If the screen didn't turn warm, use Fallback mode. | macOS 26 da Apple xatosi tufayli Dimit ba'zi Mac'larda ekran ranglarini o'zgartira olmasligi mumkin. Ekran iliq tus olmagan bo'lsa, Zaxira rejimdan foydalaning. | В macOS 26 из-за ошибки Apple Dimit на некоторых Mac не может менять цвета экрана. Если экран не стал теплее, включите Резервный режим. |
| banner.use_fallback *(added 2026-09-10)* | Use Fallback mode | Zaxira rejimni yoqish | Включить Резервный режим |
