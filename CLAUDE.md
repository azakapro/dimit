...
- **As found in the field (docs/QA.md § "Apple's macOS 26 colour-table bug"):** Apple's bug is real and documented — DTS reproduced the API being silently ignored with read-back returning the stored values (so no in-process check can detect it, as this section predicted), f.lux users swap colour profiles nightly on 26.5.1, and thread 819331 adds that on XDR panels toggling auto-brightness can leave the table "looking permanently bad", which is why the in-app advice is worded as something that helps on many machines rather than a fix. **We have never reproduced it on our own hardware.** A 26.6.2 tester reported colour changes failing on 2026-09-10 and, on the same Mac and OS build, everything working on 2026-09-11; the first report was withdrawn at the maintainer's request. That single non-reproduction is what demoted the banner to a disclosure (see the verification step above), and it is why the README describes FB18559786 / FB19136488 / FB22273730 on Apple's evidence, not ours. `ColorSyncDeviceSetCustomProfiles()` is the one alternative path the threads mention — unconfirmed, system-wide, and not attempted.
...
- Auto-brightness: we cannot disable it without root, and no detection key exists; §3.3's "Colours not changing?" disclosure is where a user affected by it is told what to do.
...
| help.colours_not_changing *(added 2026-09-11; uz/ru need human review)* | Colours not changing? | Ranglar o'zgarmayaptimi? | Цвета не меняются? |
| ~~banner.dismiss~~ *(added C3, removed 2026-09-11 with the banner — a disclosure has nothing to dismiss)* | — | — | — |
