...
- `AppState` (single `@Observable` class): `isOn, warmthK, brightness, pwmSafe, activePreset, perDisplayOverrides, schedule, hotkeys, launchAtLogin, updateChecks, locale`. Persist in `UserDefaults` (suite `PRODUCT_BUNDLE_ID`), debounced 250 ms. Apply pipeline is a single pure function `render(state, displays) -> [DisplayCommand]` so it is unit-testable.
