...
/// is the only place the question is ever asked.
///
...
final class UpdateController {
    ...
    convenience init(appState: AppState) {
        ...
        self.init(appState: appState, updater: controller.updater)
    }

    /// Injectable for tests — the mapping below is what needs pinning, not
    /// Sparkle itself.
    init(appState: AppState, updater: UpdaterControlling) {
        self.updater = updater
        // `.receive(on:)` because `@Published` fires from `willSet`: a
        // synchronous sink reading `appState` would see the old value —
        // the same trap DisplayCoordinator and ScheduleCoordinator hit.
        // The published value itself is used, so this is belt and braces.
