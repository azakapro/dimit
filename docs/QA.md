...
One row per check per machine. Fill by hand; a PR that claims a check passed must have its row here. Dated rows preserve the evidence from that build, including Fallback mode before its removal; they do not claim that a later build was retested. Pending checks remain pending unless a later row explicitly closes them.

## Gamma spike (C0)

| Date | Machine | macOS | Auto-brightness | `set` return | read-back green max | Screen turned red? |
|---|---|---|---|---|---|---|
| 2026-09-09 | MacBookPro18,1 M1 Pro | 27.0 (26A5416b) | ON | 0 (success) | 0.0 | **pending — needs human eyes, structurally.** Tried two ways to verify automatically: (1) raw `screencapture` CLI failed outright (`could not create image from display`, no Screen Recording TCC grant); (2) a computer-use screenshot taken 2s into the red window *succeeded* but showed normal full-color menu-bar icons, not red. That is not a bug — macOS screen-capture composites from the pre-LUT framebuffer, so `CGSetDisplayTransferByTable` changes are invisible to *any* screenshot API by design (this is the same mechanism CLAUDE.md rule 4 relies on for "screenshots aren't tinted"). So no software check, with any permission, can ever confirm this — only a human looking at the physical panel can. Maintainer: please run `swift scripts/gamma_spike.swift` yourself and confirm the screen visibly flashed red for ~4s. |
| | MacBookPro18,1 M1 Pro | 27.0 (26A5416b) | OFF | | | pending — also needs "Automatically adjust brightness" turned off in System Settings → Displays first (a system-setting change left to the maintainer, not done automatically). |
...
| Idle CPU | pass (quick check, not the full 5-min average) | `ps` samples at launch and 6s later: 0.0% both times. Maintainer should still confirm with a real 5-minute Activity Monitor average per CLAUDE.md §8. |
...
| Reconfiguration callback actually *fires* on a real plug/unplug | **pending** | No external monitor connected during this session. Maintainer: plug in the home monitor with the filter ON and confirm it picks up the warmth/dim within ~300ms, and that unplugging doesn't crash the app. |
| Sleep/wake re-applies within ~1s | **pending** | Can't safely trigger real system sleep in an unattended session. Maintainer: put the Mac to sleep and wake it with the filter ON, confirm the tint is still there (or reapplies) within about a second. |
...
### Maintainer-run live test on the real machine, 2026-09-09 (closes three rows that unit tests could not)

The maintainer ran the Debug build and drove the real UI while the hardware backlight, the live gamma table and the persisted state were sampled together every 1.5–2 s. This is the first evidence for these rows that isn't a unit test or a fake backend.

| Check | Result | Evidence |
|---|---|---|
| Brightness key press → re-pin within 5 s | **pass — was "pending", now closed** | Maintainer pressed F1 with PWM-Safe on: backlight fell `0.937 → 0.625 → 0.312`, and the app forced it back to `1.000` about 6 s later (poll interval is 5 s). Reproduced three times in two separate traces (`0.937 → 1.000`, `0.812 → 1.000`, `0.312 → 1.000`). |
...
One thing deliberately **not** concluded: during a stretch of rapid manual key presses the backlight sat at `0.375` for ~8 s without re-pinning. It did not reproduce in a hands-off window, and the most likely explanation is simply the maintainer's key presses outpacing the 5 s poll. Recorded here rather than fixed, because the evidence doesn't yet support a change. If a beta tester reports "PWM-Safe lets my brightness drift", start here.
...
| Every hotkey works while another app is frontmost | **pending — maintainer only** | Posting synthetic key events needs Accessibility, which this session doesn't have and the app must never request (CLAUDE.md §1.5). What *is* verified: the six handlers are one-line calls into `AppState.cycleToNextPreset` / `adjustWarmth` / `adjustBrightness`, each unit-tested including clamping at both ends. Maintainer: with Safari frontmost, press ⌃⌥⌘Z and watch the menu-bar icon flip; then record a shortcut for "Cycle Presets" in Settings and press it. |
| Login item toggles and survives reboot | **pending — maintainer only** | `SMAppService.mainApp` must be called from inside the running app, and doing it from an unattended session would register a DerivedData path as a login item. Maintainer: from a copy in /Applications, toggle it in Settings → General, confirm it appears in System Settings → General → Login Items, reboot. |
...
| VoiceOver reads every control in three languages | **pending — maintainer only** | Labels are present by construction (sliders and icon buttons carry `accessibilityLabel`; Settings/onboarding use native `Toggle`/`Picker`/`Button` whose visible text is their label). Turn VoiceOver on, open the popover and Settings, switch language, listen. |
### Maintainer-run capture test, 2026-09-09 — the §1.4 promise, verified for the first time

Real Zoom call with screen sharing, plus a QuickTime screen recording, on this machine (macOS 27.0 beta, built-in display), driven by the maintainer with the v0.2 build.

| Check | Result | Notes |
|---|---|---|
| Filter keeps working on the physical screen during a Zoom share | **pass** | Maintainer: "when screen shared it was working ok". |
| Zoom viewers see normal, untinted colours | **pass** | The share was not tinted. |
| QuickTime screen recording is not tinted | **pass** | Maintainer: "in the video no any effects" — no tint in the recorded file. |
...
| Stale debug instance | note | The maintainer's Xcode run of the 0.1 build (PID 69866) was still alive at the end of this session, paused in lldb on a SIGTERM. Stop it in Xcode (⌘.) before running 0.2 — two instances fight over the gamma table and the status bar. |
...
| Real sleep/wake interaction with an active schedule; DDC (Opus 5 half, separate PR) | **pending** | Same standing limitations as every prior cycle — needs the maintainer's hands, or the DDC half specifically needs Opus 5 and the home monitor. |
...
| Item | Status |
|---|---|
| **Does any of it actually work on a monitor?** Pin, verify, restore, the whole point. | **Unverified.** Needs the maintainer's home monitor. This is the single thing that would close the cycle. |
...
### Closing the DDC question for this monitor (2026-09-10, morning — maintainer away, monitor left connected)
...
**Conclusion:** the panel's DDC/CI *command* channel is switched off or absent — the classic OSD "DDC/CI: Off" signature (EDID works, every 0x37 command gets a null or error frame). Dimit's `unsupported` verdict is correct for this monitor as configured; there is no code change that would make it answer. **What would change it:** the maintainer opening the monitor's menu, finding DDC/CI, setting it to On, and re-running `scratchpad/ddcprobe/probe` (or simply toggling PWM-Safe with DDC enabled in Settings → Displays). Until someone does that, this monitor stays an `unsupported` data point, not a failure.
...
| **Unplug/replug while ON re-applies** (C2 "Done when") | **pass, with timings** | A watcher sampled the external table every 100 ms while the maintainer replugged: `04.483` display gone → `09.790` back with the stale 2700K table → `10.030` WindowServer resets it to neutral → `10.135` **Dimit re-applies 2700K, 105 ms after the reset** (the 300 ms debounce is the ceiling, not the typical). Dimit survived the disconnect without a crash or a stuck table. |
| Both displays follow one set of controls | pass, by design | Maintainer noticed changing warmth on either screen changes both and asked whether that's right. It is: CLAUDE.md §1.1 and §3.3 — one state, all displays; per-display overrides are a 1.1 item (§10). |
| Backlight pin/restore on the built-in with a second display attached | consistent, not conclusive | Read 1.0 before, during and after — the maintainer runs the panel at 100%, so the pin was a no-op. C3's rows already proved pin and exact restore from 0.4559. |
...
| Set VCP 0x10 = 100 (the exact PWM-Safe command, spec vector checksum `0xCC`) | acknowledged by the bus; **effect not clearly established** — a later 20→100→20→100 write sequence (both seeds) got an "I think working ok" from the maintainer, not a definite dim/brighten observation, and the OSD's DDC/CI setting was never confirmed. Treat writes as unverified until someone watches the panel during that sequence. | `IOAVServiceWriteI2C` returned success and the monitor answered null. Whether the panel actually went to 100% only the OSD can say — **maintainer: check the OSD's brightness value and whether it has a DDC/CI toggle (Xiaomi ships one, often off).** If DDC/CI was off, this whole table is "off" behaviour and needs re-running once. |
...
| Full system sleep → wake with the schedule active | **pass — maintainer-run, 2026-09-10** | Maintainer slept and woke the Mac with the schedule on and both displays attached: "sleep wake working". Closes the row that C2 opened and the C4 review flagged as never actually closed. |
| Quit restores both displays; maintainer's saved state restored afterwards | pass | Both `(0.500, 0.500, 0.500)` after SIGTERM; `state.v1` re-imported from the pre-test backup. |
...
### Capture matrix (C4 review item; the README may only claim what this table shows)
...
| **The opt-in path actually fires, and only then** | **pass — definitive both ways** | The production appcast was not reachable during this check, so a copy of the build was pointed (Info.plist patched, re-signed ad-hoc) at a local `http.server` serving a minimal appcast, with Sparkle's last-check date back-dated so a check was due. **Opt-in off, 60 s: 0 requests. Opt-in on, 60 s: exactly one `GET /appcast.xml`.** Same binary, same schedule state; the only difference was the checkbox. |
...
| `scripts/make_appcast.sh` | written; refuses to run here | Requires a Developer-ID-signed app (an ad-hoc build in the appcast would be refused by Gatekeeper on the user's Mac); exited before writing feed output. This is the original C7 script result; the GitHub Pages output path needs its own release verification. |
...
| Historical C7 product-copy claims | pass, by review of that copy | Capture claim limited to ≥ 30% brightness (this file § C6 capture matrix); DDC described as experimental and unconfirmed; PWM described as a mechanism, no health claim (CLAUDE.md §1.7); tested-on list is exactly the hardware in this file. |
| Notarized v1.0 | **pending — maintainer** | No Developer ID on this machine (§ C6). |
| Manual "Check for Updates…" before the feed exists | known, documented | With the production feed unreachable during C7, a manual check showed Sparkle's own "update error" dialog — in Sparkle's localizations, which include `ru` but not `uz`, so Uzbek users see English. docs/RELEASE.md makes a reachable appcast a tagging gate; nothing to fix in the app. |
| App strings still English in uz/ru (47 keys: Settings tabs, hotkeys, schedule, onboarding, Restore Colours, DDC help) | translated, **review pending** | Written during C7 and left in `needs_review` so the catalog editor flags them (CLAUDE.md §5.1's mechanism). A fluent reviewer must review these app strings; this row does not mark that review complete. |

**`/code-review high`, 8 angles, 10 findings, all fixed before the original C7 merge.** App and release-tool findings preserved here: the “Check for Updates…” enable state was dead code (NSMenu auto-enables items and re-derives the state on pop-up; fixed with `autoenablesItems = false`); `make_appcast.sh` would have committed and deployed every old ZIP and delta forever (archives were moved out of tracked source, deltas disabled and `old_updates/` removed); and uz/ru Settings strings were still English (translations added and flagged for fluent review). Also: `SPUStandardUpdaterController` is now retained rather than dropped after `init`; tool lookup prefers the DerivedData used by `build.sh`; tests assert that opting in never starts a check by itself; product copy no longer described an unnotarized 0.4 build as notarized; and the Mi Monitor finding was described as “unconfirmed”, not “not supported”. These are historical review results, not verification of the current GitHub Releases/Pages flow.


## Maintainer-reported UI bug: the OFF button's label was unreadable (2026-09-10)

The maintainer ran the C7 build, looked at the popover and said: *"when it is off i can't see the letters."* The screenshot showed a solid black ON/OFF button with "OFF" barely visible on it. Nothing in 183 tests had caught it, because every render test in `LayoutRenderTests` measured **geometry** and none had ever looked at a **colour**.
...
The ON state's 3.44:1 clears WCAG's 3:1 bar for large text (17pt bold) but not the 4.5:1 normal-text bar. That ceiling belongs to the specified brand colours, so the test's floor is the large-text bar and raising it is a palette decision for the maintainer, not a silent edit.

**Two things this fix could not verify here, both needing the maintainer's hands:**

| Item | Status |
|---|---|
| **Is the whole OFF button clickable?** It no longer has an opaque fill, and a SwiftUI control's hit region depends on how a `.clear`-filled shape is treated. | **Handled, not measured.** Three attempts to observe the real hit region failed: `NSHostingView.hitTest` returns nil for every point when the view isn't in a window, and returns the host view itself for every point when it is — giving the *same* answer for the opaque ON state, so it cannot discriminate. Closed instead by declaring `.contentShape(shape)` in the style, which defines the hit area explicitly. Maintainer: click the OFF button near its left edge, not the centre, and confirm it toggles. |
| **Does the button still show a keyboard focus ring?** It moved from a system `.borderedProminent` style to a custom `ButtonStyle`, and custom styles do not necessarily inherit the system focus effect. CLAUDE.md §8 requires "full keyboard operation". | **Open.** Not verifiable off-screen. Maintainer: System Settings → Keyboard → turn on "Keyboard navigation", open the popover, press Tab until the ON/OFF button is focused, and confirm a visible ring appears. If it doesn't, the fix is a `@FocusState`-driven ring in `ZapButtonStyle` — real work, so it is recorded here rather than guessed at now. |

## Maintainer-reported UI bug: the menu-bar icon grew when PWM-Safe pinned (2026-09-10)
...
`scripts/build.sh` produced an app that passed every check the script made — valid signature, universal, no entitlements, hardened runtime on — and **died before `main()`** when the maintainer opened the copy installed in `/Applications`. Three crash reports, all the same:
...
## Maintainer walkthrough of Settings (2026-09-10) — four reports, one real bug

The maintainer used the app and asked four questions. Three had the same shape: the feature worked, and the interface said something false or nothing at all. Recorded because "it doesn't work" and "it doesn't say what it's doing" arrive from users identically.

| Reported | What was actually true | Change |
|---|---|---|
| *"it asked my location but didn't show it after"* | **Real bug.** CoreLocation returned coordinates near Tashkent (precise device location redacted) and they were stored correctly, but `currentLocationDescription` had only two branches — city, or "Using manual coordinates" — so a device fix displayed as *manual coordinates* and named nowhere. The City row simultaneously read "No location set", contradicting the line under it. | Shows the nearest city by name, resolved **offline** against the bundled list (`CityList.nearest`, 75 km cap, haversine). Reverse geocoding would name it exactly but is a network call, and CLAUDE.md §4.3 allows the app one, for Sparkle, on opt-in. Verified by rendering the tab with that location result: **"Using your location · Tashkent"**. The picker's empty row now reads "Choose a city" so the two lines agree. |
...
Five tests cover `CityList.nearest`, including the Tashkent location case, the Fergana-valley case where three cities sit within ~80 km of each other, a mid-Pacific point that must name nothing rather than guess, and an antimeridian point that must not wrap onto London or Tokyo. 187 → 192.

### Launch at login — resolved, and what it cost to find out (2026-09-10)

**Maintainer-confirmed working** from `/Applications/Dimit.app`, on the build with the hardened-runtime fix. No crash reports since.
...
The maintainer deleted the app, installed the new build, and found Fallback mode still on — reasonably asking whether a first launch defaults to it, and offering to have the feature removed if not fixable.
...
## Fallback mode removed (2026-09-10, maintainer decision)

After the 26.6.2 report, Fallback mode went from a Settings toggle to a one-tap banner button to a visible popover row in the space of a few hours — and the maintainer, using the app, found the whole thing confusing and asked for it to go. The engineering tradeoff was recorded: f.lux users affected by the same Tahoe bug swap colour profiles manually, and the overlay code remains shared with the dim floor. The maintainer reaffirmed the removal. Removed: `AppState.fallbackMode`, `RenderState.fallbackMode`, `PersistedState.fallbackMode`, `OverlayTint` (the overlay is now always black), `Config.fallbackMaxWarmthVeil`, the Advanced toggle, the right-click menu item, the popover row, the banner button, seven strings, and ten tests that covered only the removed branch.
