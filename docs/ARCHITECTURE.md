...
- **No `source` field on `AppState`.** The sketch above proposes tagging a write `source = .schedule`; the shipped mechanism instead has `ScheduleCoordinator` set a private flag immediately around its own two writes and treat any `warmthK`/`brightness` change that arrives *without* that flag set as a manual override — cheaper than adding a field `AppState` itself has no other use for, and `AppState` stays a plain value bag with no knowledge that scheduling exists.
- **`target()` returns only warmth/brightness, never an on/off state.** A schedule that could switch the filter on by itself (say, mid screen-share, after sunset) is a worse surprise than one that only ever steers what the filter would show once the user has it on. The user's ON/OFF toggle stays theirs alone; the schedule and the toggle are independent axes, matching how the popover's sliders already behave while OFF.
...
- **The driver's timer runs in `.common` run-loop mode from the start**, not `.default`. A `.default`-mode timer stops firing while a menu is open or a window is being dragged — the independent C4 review found precisely this bug in `PWMSafeCoordinator`'s drift poll (still open as of this cycle, docs/QA.md); `ScheduleCoordinator` uses `.common` from day one rather than needing its own review cycle to catch the same class of bug twice.
